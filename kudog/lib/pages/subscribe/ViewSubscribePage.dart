          ...
          child: Column(children: [
            ...
            Expanded(
              child: ListView.builder(
                  itemCount:
                      noticeService.subscribeList.length == 0 || isEditting
                          ? noticeService.subscribeList.length + 1
                          : noticeService.subscribeList.length,
                  itemBuilder: (context, index) {
                    return index == noticeService.subscribeList.length
                        ? Container(
                        ...
                        : GestureDetector(
                            onTap: () {
                              if (isEditting)
                                setState(() {
                                  selectedLists.contains(noticeService
                                          .subscribeList[index].id!)
                                      ? selectedLists.remove(noticeService
                                          .subscribeList[index].id!)
                                      : selectedLists.add(noticeService
                                          .subscribeList[index].id!);
                                });
                              else
                                Navigator.push(
                            ...
                            child: SubscribeCard(
                                subscribe: noticeService.subscribeList[index],
                                selected: selectedLists.contains(
                                    noticeService.subscribeList[index].id)));
                  }),
            ),
          ]),
        ),
